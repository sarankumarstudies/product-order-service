...
public class OrderController {
    ...
    public Mono<ResponseEntity<OrderResponseDto>> placeOrder(@RequestBody Order order) {
        ...
        return webClientBuilder.build().get().uri("http://localhost:8081/products/" + order.getProductId()).retrieve().bodyToMono(ProductResponseDto.class).map(productResponseDto -> {
